...
// â”€â”€ Pipeline stage sequence (exact Arivu pipeline) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// STREAMS: 6 Binance WS feeds → tick merge

// BARS:    ticks accumulate 10s → bar close → 19 variables computed → adaptive PCMCI refresh

// PCMCI:   graph discovery (PCMCI, τ_max/α driven by MetaOptimizer hill-climb) → GraphSnapshot

// HYPOTHESIS: walk chains to price_return → rank top-5 → composite score (geometric mean for multi-hop)

// OPTIMIZER:  MetaParameterOptimizer hill-climbs 5 dims: k_runs/min_runs/threshold/τ_max/α per regime
...
// â”€â”€ Layer config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ 19 VARIABLE_NAMES (exact order from feature_bar.py) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ 6 Binance WS streams and what they produce â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

const WS_STREAMS = [

  { name:'kline_1m',      col:'#7c3aed', fields:'price · volume (OHLCV)',         vars:['price_return','volume'] },

  { name:'bookTicker',    col:'#4f46e5', fields:'bid · ask  →  spread',            vars:['spread','ema_spread'] },

  { name:'aggTrade',      col:'#2563eb', fields:'trade side aggression',            vars:['trade_intensity'] },

  { name:'depth5@100ms',  col:'#0891b2', fields:'L2 order book  →  imbalance',     vars:['order_book_imbalance'] },

  { name:'btcusdt',       col:'#f59e0b', fields:'BTC macro price  →  btc_return',  vars:['btc_return'] },

  { name:'ethusdt',       col:'#ef4444', fields:'ETH macro price  →  eth_return',  vars:['eth_return'] },

]

// â”€â”€ 5 causal layers (groups of VARIABLE_NAMES) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Mock fallback (real WS field names: ts, price, spread, volatility, rsi) â”€â”€â”€
...
const MOCK_HYPOTHESIS = {
  ...
  chain_summary:()=>'btc_return→(lag=1)→price_return',
...
const abbr = n => n.length>12 ? n.slice(0,5)+'…'+n.slice(-3) : n

// Robustly extract "source|target" edge pairs from any hypothesis format:

//   Real JSONL → chain_edges:[{source,target}]  OR  chain:"A→(lag=1)→B" string

//   Mock       → chain:[{source,target}] array
...
function chainSet(c){
  ...
  segments.forEach(seg => {
    // Each segment: "source->(lag=N)->target" or "source→(lag=N)→target"
    // Split on -> or → (with optional whitespace), then strip (lag=N) and dashes
    const parts = seg
      .split(/\s*-?[→>]-?\s*/)               // split on -> or → (with optional -)
...
// â”€â”€ Draw primitives â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function drawLivePanel(ctx,W,H,ticks,ld){
  ...
  ctx.fillText(`RSI ${ld.rsi?.toFixed(1)||'—'}  σ ${ld.volatility?.toFixed(5)||'—'}`,px+10,py+60)
  ...
  ctx.fillText('HYP: '+(ld.selected_chain||'—').slice(0,28),px+10,py+102)

  ctx.fillStyle=ld.hold_reason==='traded'?'#16a34a':'#f59e0b'

  ctx.fillText(ld.hold_reason==='traded'?'▲ TRADE':('● '+(ld.hold_reason||'observing')).slice(0,26),px+10,py+115)

  ctx.restore()

}

// ── Layer column backgrounds (for graph stages) ───────────────────────────────
...
export default function LiveGraph({
  ...
  // Stage pill → pipeline index mapping
  ...
  drawFn.current = useCallback((ts)=>{
    ...
    // STAGE 1: STREAMS — 6 Binance WS feeds → tick merge
    ...
    if(vsName==='STREAMS'){
      ...
      if(mA>0){
        ...
        const TCOLS=['TIME','PRICE','SPREAD','VOL·σ','RSI']
      ...
      stageLabel(ctx,W,H,'STREAMS — Binance WebSocket Feeds',sT)

    }

    // ════════════════════════════════════════════════════════════════

    // STAGE 2: BARS — tick accumulation → 10s bar close → 19 features
    ...
    // ════════════════════════════════════════════════════════════════
      ...
      if(sT<0.32){
        ...
        // ACT B: bar closed → 19 variable names cascade in groups
      ...
      // Draw background — highlight the active layer column with a bright pulse
      ...
      LAYER_PHASES.forEach(({l,t0,t1})=>{
        ...
        layerEdges.slice(0,nShow).forEach((e,ei)=>{
          ...
          if(e.validated&&(isNew||Math.abs(e.coeff)>0.4)){
            ...
            ctx.fillText(`lag=${e.lag}  β=${e.coeff?.toFixed(2)}`,lx,ly)
        ...
        if(isActiveLayer&&layerT<0.95){
          ...
          ctx.fillText(`↓ ${l.toUpperCase()} LAYER`,lx3,ly3)
      ...
      stageLabel(ctx,W,H,'PCMCI — Layer-by-Layer Discovery',sT)

    }

    // â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• 

    // STAGE 4: HYPOTHESIS — ALL ranked chains visible simultaneously
    ...
    // â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• 
      ...
      // Colors + glow by rank (0=best) — 12 entries for up to 12 hypotheses
      const HYP_COLS=[
      ...
      // Build per-hypothesis edge sets (robustly parsed) — all hypotheses, no cap
      ...
      ;[...hypList].reverse().forEach(({h,pairs,col,glow,lw})=>{
        ...
        hlEdges.forEach(e=>{
          ...
          if(glow>=0.5){
            ...
            ctx.fillText(`lag=${e.lag}  β=${e.coeff?.toFixed(3)||'?'}`,(e.sx+e.tx)/2+(e.sy<e.ty?-22:22),(e.sy+e.ty)/2)
      ...
      hypList.forEach(({h,hi,col,pairs})=>{
        ...
        const rawChain=typeof h.chain==='string'?h.chain
          ...
          :Array.isArray(h.chain_edges)?h.chain_edges.map(e=>`${abbr(e.source)}→${abbr(e.target)}`).join(' ')

          :Array.isArray(h.chain)?h.chain.map(e=>`${abbr(e.source)}→${abbr(e.target)}`).join(' '):''

        const dispChain=rawChain.replace(/\(lag=\d+\)→/g,'→').slice(0,24)

        ctx.fillText(`H${hi+1} ${dispChain}`,legX+20,ry+9)

        if(!pairs.size) { ctx.fillStyle='#94a3b8'; ctx.fillText('— no match',legX+20,ry+9) }
      ...
      stageLabel(ctx,W,H,'HYPOTHESIS — Chain Selection',sT)
      ...
      ctx.font='700 12px JetBrains Mono,monospace'; ctx.fillStyle='#334155'; ctx.textAlign='center'

      ctx.fillText(`META-OPTIMIZER  \u00b7  REGIME: ${activeR.toUpperCase()}  \u00b7  HILL CLIMB`,W/2,22)

      ctx.font='600 8px JetBrains Mono,monospace'; ctx.fillStyle='#94a3b8'

      ctx.fillText('X = hill-climbing iterations  \u00b7  Y = score  \u00b7  particle speed \u221d k_runs',W/2,36)

      // Panel layout

      const PAD_L=54  // Y axis space
      ...
      const totalPH=H-TOP-22

      const pH=Math.floor((totalPH-GAP*2)/3)

      const pW=W-PAD_L-PAD_R

      // Score landscape: x [0,1] → score [0,1]
      ...
      const makeCurve=(cand,ci)=>{
        ...
        const peakH=0.20+thN*0.60      // threshold → how high the score ceiling is

        const rise =3+mnN*11           // min_runs  → sigmoid steepness

        const noise=0.01+(1-alN)*0.14  // alpha     → noise (lower α = smoother)

        const bF   =4+ci*1.9+(1-mnN)*3
      ...
      candList.forEach((cand,ci)=>{
        ...
        ctx.shadowColor=col+'18'; ctx.shadowBlur=10

        ctx.beginPath(); ctx.roundRect(PAD_L,panelY,pW,pH,6); ctx.fill()

        ctx.shadowBlur=0; ctx.strokeStyle=col+'44'; ctx.lineWidth=1; ctx.stroke()
        ...
        ctx.roundRect(PAD_L,panelY,3,pH,{upperLeft:6,lowerLeft:6,upperRight:0,lowerRight:0})

        ctx.fill()

        // Title strip background

        ctx.fillStyle=col+'11'; ctx.fillRect(PAD_L+3,panelY,pW-3,18)

        // Title

        ctx.font='700 8px JetBrains Mono,monospace'; ctx.fillStyle=col; ctx.textAlign='left'

        ctx.fillText(`C${ci}`,PAD_L+10,panelY+12)

        ctx.font='500 7px JetBrains Mono,monospace'; ctx.fillStyle='#334155'

        const ps=`k_runs=${cand.k_runs??'\u2014'}  thresh=${cand.threshold?.toFixed(3)??'\u2014'}  min=${cand.min_runs??'\u2014'}  \u03c4=${cand.tau_max??'\u2014'}  \u03b1=${cand.pcmci_alpha?.toFixed(3)??'\u2014'}`

        ctx.fillText(ps,PAD_L+30,panelY+12)

        if(cand.recent_score!=null){

          ctx.font='700 8px JetBrains Mono,monospace'; ctx.fillStyle=col; ctx.textAlign='right'

          ctx.fillText(`score ${cand.recent_score.toFixed(4)}`,PAD_L+pW-6,panelY+12)

        }

        // ── Plot area ──

        const gX=PAD_L+4, gY=panelY+20, gW=pW-8, gH=pH-26
        ...
        for(let i=0;i<=NPTS;i++){
          ...
          const x=i/NPTS, y=fn(x)

          cpts.push({px:gX+x*gW, py:gY+gH-y*gH})
        ...
        ctx.strokeStyle=col+'bb'; ctx.lineWidth=1.8; ctx.stroke()

        ctx.restore()

        // Peak guide line (dashed)
        ...
        ctx.fillText(peakH.toFixed(2),PAD_L-3,peakPY+3)  // peak score label

        ctx.fillText('0.00',PAD_L-3,gY+gH+3)             // zero label

        // Y axis title rotated

        ctx.save(); ctx.translate(PAD_L-14,gY+gH/2); ctx.rotate(-Math.PI/2)

        ctx.font='600 6px JetBrains Mono,monospace'; ctx.fillStyle='#94a3b8'; ctx.textAlign='center'

        ctx.fillText('score',0,0); ctx.restore()

        // ── ANIMATED PARTICLE (speed proportional to k_runs) ──

        const speed=0.18+kN*0.82   // 0.18 (k=0) to 1.0 (k=60)

        const pX   =((sT*speed+ci*0.30)%1)

        const pY   =fn(pX)

        const pdx  =gX+pX*gW

        const pdy  =gY+gH-pY*gH
        ...
        for(let ti=TRAIL;ti>0;ti--){
          ...
          const trX=Math.max(0,pX-ti/TRAIL*0.22)

          const trY=fn(trX)

          ctx.globalAlpha=(1-ti/TRAIL)*0.48

          ctx.beginPath()

          ctx.arc(gX+trX*gW,gY+gH-trY*gH,2.0,0,Math.PI*2)
        ...
        ctx.beginPath(); ctx.arc(pdx,pdy,11,0,Math.PI*2)
        ...
        // Score label at particle

        ctx.font='700 7px JetBrains Mono,monospace'; ctx.fillStyle=col

        ctx.textAlign=pX>0.82?'right':'left'

        ctx.fillText(pY.toFixed(3),pdx+(pX>0.82?-11:11),pdy-7)

        // Speed badge

        ctx.font='500 6px JetBrains Mono,monospace'; ctx.fillStyle=col+'88'; ctx.textAlign='right'

        ctx.fillText(`${Math.round(speed*100)}% spd (k=${cand.k_runs??'?'})`,PAD_L+pW-5,panelY+pH-4)

      })

      // X axis label

      ctx.font='600 7.5px JetBrains Mono,monospace'; ctx.fillStyle='#94a3b8'; ctx.textAlign='center'

      ctx.fillText('hill-climbing iterations →',W/2,H-10)

      // Legend

      const legL=['k_runs → speed','threshold → height','min_runs → steepness','α → noise']

      ctx.font='500 6.5px JetBrains Mono,monospace'; ctx.textAlign='left'

      legL.forEach((t,i)=>{ ctx.fillStyle=CCOLS[i%3]+'99'; ctx.fillText(t,PAD_L+i%2*(pW/2),H-2) })
      ...
      stageLabel(ctx,W,H,'OPTIMIZER — Score Landscape Curves',sT)

    }

    // ════════════════════════════════════════════════════════════════════════════

    // STAGE 6: TWIN — expected causal path vs actual market path
    ...
    // ════════════════════════════════════════════════════════════════════════════
      ...
      if(displayEdges.length){
        ...
        // Default to aligned when we can't measure — the hypothesis passed ranking to reach here

        const sameDir = actualDir == null ? true : actualDir === predDir

        const actualOff=sameDir?divAmp*0.18:divAmp

        // — EXPECTED path: indigo particle
        ...
        // — ACTUAL path: amber particle, offset in y
        ...
        if(pn&&best){
          ...
          ctx.fillText(`${predDir==='up'?'↑':'↓'} ${((predMag||0)*100).toFixed(3)}%  `,pn.x,pn.y-30)
      ...
      stageLabel(ctx,W,H,'TWIN — Expected vs Actual',sT)

    }

    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    // STAGE 7: DECISION — trade committed or hold (with reason)
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
      ...
      ctx.fillText(traded?'â–² TRADE EXECUTED':isEV?'âš¡ ESCAPE VALVE FIRED':'â— HOLD',dx2+12,dy2+13)
      ...
      stageLabel(ctx,W,H,'DECISION — Trade or Hold',sT)

    }

    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    // STAGE 8: LEARNING — ML1 retrain, Layer2 trust update, MetaOptimizer step
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
      ...
      ctx.fillText('LEARNING  ·  Outcome → Feedback Loop',W/2,22)

      ctx.font='600 8px JetBrains Mono,monospace'; ctx.fillStyle='#94a3b8'

      ctx.fillText('price_return resolves → ML1 retrains + edge trust updates + MetaOpt hill-climbs',W/2,36)

      // ── Animated feedback arcs: from price_return backwards to learning targets ──
      ...
      if(pn){
        ...
        feedbackTargets.forEach((ft,fi)=>{
          ...
          // Highlight target node — glow halo + bright fill + name
          if(tn){
      ...
      // ── HUD panel: slim 3-row info strip ──────────────────────────────────
      ...
      ctx.fillText(topEdge2?`top: ${topEdge2.source}→${topEdge2.target} trust=${topEdge2.trust?.toFixed(3)||'—'}$}`:'no outcomes yet',ml1x+20,r2y)
      ...
      ctx.fillText(lastScore2!=null?`score ${lastScore2.toFixed(4)} ${improving2?'↑':'→'}`:'exploring…',moX+32,r2y)
      ...
      stageLabel(ctx,W,H,'LEARNING — Feedback & Weight Updates',sT)
